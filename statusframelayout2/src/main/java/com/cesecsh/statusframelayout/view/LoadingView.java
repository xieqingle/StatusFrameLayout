...
import com.cesecsh.statusframelayout.R;
...
public class LoadingView extends View {
    ...
    private void init(AttributeSet attrs) {
        ...
        mDuration = typeArray.getInt(R.styleable.LoadingView_lv_duration, DEFAULT_DURATION);
        mInternalR = typeArray.getDimension(R.styleable.LoadingView_lv_internal_radius, DEFAULT_INTERNAL_RADIUS);
        mExternalR = typeArray.getDimension(R.styleable.LoadingView_lv_external_radius, DEFAULT_EXTERNAL_RADIUS);
        int defaultColor = 999999;
        int startColor = typeArray.getColor(R.styleable.LoadingView_lv_start_color, defaultColor);
        int endColor = typeArray.getColor(R.styleable.LoadingView_lv_end_color, defaultColor);
        ...
        if (colorList.size() == 0) {
            mColors = new int[]{Color.BLUE, Color.RED};
        } else {
    ...
    private void drawBezier(Canvas canvas) {
        ...
        if (isEvenCyclic()) {
            if (mAngle % mRadian < mRadian / 2) {
                ...
                mPath.quadTo(rightX + (leftX - rightX) / (mRadian / 2) *
                                (mAngle % mRadian > (mRadian / 2) ? (mRadian / 2)
                                        : mAngle % mRadian),
                        rightY + (leftY - rightY) /
                                (mRadian / 2) *
                                (mAngle % mRadian > (mRadian / 2) ? (mRadian / 2) :
                                        mAngle % mRadian), pointF2.x, pointF2.y);
                mPath.lineTo(pointF3.x, pointF3.y);

                mPath.moveTo(pointF4.x, pointF4.y);
                mPath.quadTo(leftX + (rightX - leftX) /
                                (mRadian / 2) * (mAngle % mRadian > (mRadian / 2) ? (mRadian / 2) : mAngle % mRadian),
                        leftY + (rightY - leftY) /
                                (mRadian / 2) * (mAngle % mRadian > (mRadian / 2) ? (mRadian / 2)
                                : mAngle % mRadian), pointF1.x, pointF1.y);
